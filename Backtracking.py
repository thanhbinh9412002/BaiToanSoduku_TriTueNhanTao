...
# Backtracking search
def backtracking_search(csp, win, select_unassigned_variable=minimum_remaining_values,
    ...
    def backtrack(assignment):
        ...
        for value in order_domain_values(var, assignment, csp):
            if csp.nconflicts(var, value, assignment)==0:
                ...
                insert_value(win, caculate_position_in_grid(var), int(value))
                time.sleep(0.01)
        ...
        insert_value(win, caculate_position_in_grid(var), 0)
        time.sleep(15)
        return None
    result = backtrack({})
...
if __name__ == '__main__':       
    ...
    init_assign = init_sudoku('hard')
    win = draw_sudoku(init_assign)
    time.sleep(1)
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                running = False
        if solve == False:
            sodoku = Sudoku(init_assign)
            a = backtracking_search(sodoku, win)
            sodoku.display()
            solve = True
